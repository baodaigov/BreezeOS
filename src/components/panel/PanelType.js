...
import {setVolume, setBrightness, toggleBluetooth, toggleWifi} from "../../reducers/settings";
...
const PanelType = props => {
    ...
    switch(props.type){
        case 'wifi':
            return (
                <div className={`Panel ${shellTheme === 'WhiteSur' ? 'whitesur' : ''} ${props.onActive ? 'active' : ''}`} style={props.style}>
                    <div style={{ position: 'relative' }}>
                        <div className='PanelTypeContainer'>
                            <div className={`wifiPanel ${props.onActive ? 'active' : ''}`}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '330px', padding: '0 10px' }}>
                                ...
                                {settings.wifi ? (
                                    <div className='WifiList'>
                                        ...
                                        <div className='WifiListItem' onClick={() => openSettings(toggleActive(true))}>
                                            <p className='WifiName'>Other...</p>
                                        </div>
                                        <div className='WifiListItem' onClick={() => openSettings(setSettings('Wi-Fi'))}>
                            ...
                            <div className={`batteryPanel ${props.onActive ? 'active' : ''}`}>
                                <div className='BatteryContent'>
                                    <div className='BatteryHeader'>
                                        <p>{batteryState.charging ? 'Charging' : batteryPercent <= 10 ? 'Low Battery' : 'Battery'}</p>
                                        <p className='BatteryLevel font-bold'>{batteryPercent}%</p>
                                    </div>
                                    <ActMenu className={props.onActive ? 'active' : ''} style={{ width: '332px' }}>
                                        <ActMenuSelector title='Battery Preferences...' onClick={() => openSettings(setSettings('Battery'))}></ActMenuSelector>
                                    </ActMenu>
                                </div>
                            </div>
                        </div>
        ...
        case 'bluetooth':
            return (
                <div className={`Panel ${shellTheme === 'WhiteSur' ? 'whitesur' : ''} ${props.onActive ? 'active' : ''}`} style={props.style}>
                    <div style={{ position: 'relative' }}>
                        <div className='PanelTypeContainer'>
                            <div className={`bluetoothPanel ${props.onActive ? 'active' : ''}`}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '330px', padding: '0 10px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <div style={{ position: 'relative' }}>
                        <div className='PanelTypeContainer'>
                            <div className={`defaultPanel ${props.onActive ? 'active' : ''}`}>
                                <div className='PanelBottom' style={{ width: '330px', padding: '10px 0' }}>
